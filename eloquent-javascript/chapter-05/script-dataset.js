...
function reduce(array, reduction, start) {
  ...
  for (let item of array) {
    current = reduction(current, item);
  }
  return current;
}

function characterCount(script) {
  return script.ranges.reduce((acc, [from, to]) => acc + (to - from), 0);
}

function printNames(scripts) {
...
function determineScript(code) {
  for (let script of SCRIPTS) {
    let contained = script.ranges.some(([from, to]) => {
      return code >= from && code < to;
    });
    if (contained) return script;
  }
  return null;
}
...
function countBy(array, groupName) {
  ...
  for (const item of array) {
    ...
    let resultIndex = result.findIndex(x => x.name === name);
    if (resultIndex === -1) {
...
function scriptProportion(text) {
  let scripts = countBy(text, char => {
    let script = determineScript(char.codePointAt(0));
    return script ? script.name : "none";
  }).filter(({name}) => name !== "none");   // directly access name property of object!

  let total = scripts.reduce((prev, {count}) => prev + count, 0);
  if (total === 0) return "No scripts found";

  return scripts.map(({name, count}) => {   // destruct object
    return `${name}: ${Math.floor(count * 100 / total)}%`;
  }).join(", ");
}

// find script with most characters
let largest = SCRIPTS.reduce((x, y) => characterCount(x) > characterCount(y) ? x : y);

// average year of origin for living/dead scripts
let avgLiving = average(SCRIPTS.filter(x => x.living).map(x => x.year));
let avgDead = average(SCRIPTS.filter(x => !x.living).map(x => x.year));

console.log(scriptProportion('英国的狗说"woof", 俄罗斯的狗说"тяв"'));
