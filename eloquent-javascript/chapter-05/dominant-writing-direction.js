...
function dominantDirection(text) {
    ...
    return directions.reduce((x, y) => x.count > y.count ? x : y).name;
}
...
function dominantDirection(text) {
    ...
    let top = scripts.reduce((x, y) => x.count > y.count ? x : y); 
    let foundAt = SCRIPTS.findIndex(({name}) => name === top.name);

    return foundAt !== -1 ? SCRIPTS[foundAt]["direction"] : null;
}
...
console.log(dominantDirection1("Hallo俄罗斯的狗说"));
